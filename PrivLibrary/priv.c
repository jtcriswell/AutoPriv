...
int priv_lowerall ()
{
    ...
    cap_v_list = (cap_value_t *)alloca(sizeof(cap_value_t) * (CAP_NUM - 1));
    ...
    // drop the last capability
    cap_value_t last_cap[1];
    last_cap[0] = CAP_LAST_CAP;
    if (cap_set_flag(cap, CAP_EFFECTIVE, 1, last_cap, CAP_CLEAR) != 0) {
        cap_free(cap);
        return -1;
    }
